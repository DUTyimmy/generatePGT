...
class PathIndex:
    ...
    def __init__(self, radius, default_size):
        ...
        self.radius_floor = int(np.ceil(radius) - 1)  # np.ceil : round up to an integer  # 9
        ...
        #     if self.src_indices[i] != i + 9:
        #         print(self.src_indices[i])
        #         print('False')
        #         j += 1
        # print(j)
        return
    ...
    def get_search_paths_dst(self, max_radius=5):
        ...
        coord_indices_by_length = [[] for _ in range(max_radius * 4)]  # in range 40

        # initialize a 40-length empty vector
        ...
        for y in range(1, max_radius):
            for x in range(-max_radius + 1, max_radius):  # from -9 to 9
                if x * x + y * y < max_radius ** 2:  # search R
                    search_dirs.append((y, x))

        for dir in search_dirs:

            length_sq = dir[0] ** 2 + dir[1] ** 2  # the current radius for this x, y
            path_coords = []
            ...
            for y in range(min_y, max_y + 1):
                for x in range(min_x, max_x + 1):
                    ...
                    dist_sq = (dir[0] * x - dir[1] * y) ** 2 / length_sq  # 求x, y 向下索引的各个数值对的均方值(radius)

                    if dist_sq < 1:
                        path_coords.append([y, x])

                        # 如果这个均方值小于x, y的均方值，则记录下来
                        # 实际上就是找x, y点作为圆周的 园内各个坐标对

            path_coords.sort(key=lambda x: -abs(x[0]) - abs(x[1]))  # 排序，升序输出
        ...
        # p[:, 0]  :  pick the first data in every columns

        return path_list_by_length, path_destinations

    # 152 is all the x, y coordinates' number in a circle whose radius is 10 (在两个象限)
